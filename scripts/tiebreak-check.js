...
const pari = (code, ids) => {
  let s = game.createGame({ code, mode: "online", hostId: ids[0], category: catalog.OFFICIAL_CATEGORIES[0], config: { budget: 20, slots: 3, maxPlayers: 5 } });
...
const otto = pari("5PLAY", ["a", "b", "c", "d", "e"]);
const cl8 = game.finalStandings(otto);
check("cinque giocatori: la classifica li ordina tutti", cl8.length === 5, cl8.length);
check("cinque giocatori: nessuna posizione doppia",
  new Set(cl8.map((r) => r.player.id)).size === 5);
check("cinque giocatori: il vincitore e' uno solo",
  cl8.filter((r) => r.player.id === cl8[0].player.id).length === 1);
const stessa = game.finalStandings(pari("5PLAY", ["a", "b", "c", "d", "e"]));
check("cinque giocatori: stesso ordine su un altro dispositivo",
  cl8.map((r) => r.player.id).join() === stessa.map((r) => r.player.id).join());

// Pareggio a tre su otto, con gli altri cinque staccati dai voti.
let tre = pari("TRE12", ["a", "b", "c", "d", "e"]);
for (const chi of ["d", "e"]) tre.players.find((p) => p.id === chi).budget = 1;
const cl3 = game.finalStandings(tre);
const primi = cl3.slice(0, 3).map((r) => r.player.id);
check("pareggio a tre su cinque: i tre pari stanno davanti",
